...
class AccNiCControllerModuleImp(outer: AccNicController)(implicit p: Parameters) extends LazyModuleImp(outer) with HasNICParameters {
 ...
 * Recv frames
 */
class AccNicRecvPath(val tapFuncs: Seq[EthernetHeader => Bool] = Nil)
...
class AccNicRecvPathModule(val outer: AccNicRecvPath)
...
class AccNIC(address: BigInt, beatBytes: Int = 8,
  ...
  val sendPath = LazyModule(new AccNicSendPath(nInputTaps))
  val recvPath = LazyModule(new AccNicRecvPath(tapOutFuncs))
